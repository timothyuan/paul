...
function getQuery(table, params) {
  ...
  var query = "SELECT candidates.name, votes.precinct_id, votes.count, precincts.city, precincts.county, race.african_american, race.asian, race.caucasian, race.hispanic, race.native_american, race.uncoded, age.a, age.b, age.c, age.d, age.e, sex.male, sex.female, sex.unknown, party.democratic, party.green, party.libertarian, party.other, party.republican, party.unaffiliated, party.peace_and_freedom FROM votes INNER JOIN candidates ON (votes.candidate_id = candidates.id) INNER JOIN precincts ON (votes.precinct_id=precincts.id) INNER JOIN race ON (precincts.id=race.precinct_id) INNER JOIN age ON (precincts.id=age.precinct_id) INNER JOIN sex ON (precincts.id=sex.precinct_id) INNER JOIN party ON (precincts.id=party.precinct_id)";
  var first = true;
